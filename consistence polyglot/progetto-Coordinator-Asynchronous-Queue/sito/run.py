...
def aggiorna_tabelle():
    ...
    stato_coda = requests.get('http://localhost:8080/codaasincrona/visualizzacoda').json()
    ...
    if stato_coda!=[]:
        stato_coda = pd.DataFrame.from_records(stato_coda)
        stato_coda.rename(columns = {'nome':'NOME', 'email':'EMAIL'}, inplace = True)
        stato_coda = stato_coda[['EMAIL', 'NOME']]
        stato_coda = stato_coda.to_html(classes='data', index_names=False, index=False)
        stato_coda = stato_coda.replace('<table border="1" class="dataframe data">','<table border="1" id="coda" class="dataframe data">')
    else:
        stato_coda = '<table border="1" id="coda" class="dataframe data"><thead><tr style="text-align: right;"><th>EMAIL</th><th>NOME</th></tr></thead><tbody></tbody></table>'

    return utenze_relazionale,utenze_non_relazionale, stato_coda

@app.route("/")
def index():
    utenze_relazionale, utenze_non_relazionale, stato_coda = aggiorna_tabelle()

    return render_template('index.html',
        tabellaRelazionale=utenze_relazionale,
        tabellaNonRelazionale=utenze_non_relazionale,
        statoCoda = stato_coda,
        )
